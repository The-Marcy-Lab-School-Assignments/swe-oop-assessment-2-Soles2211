...
*   Encapsulation is akin to using a smartphone daily. To use a phone, you don't need to know how the circuits, wires, plates, and transistors connect and function together. You only need to turn it on and tap the screen to get it to function.
...
*   In OOP, polymorphism means being able to handle objects the same way despite differences in the implementation of methods because the objects share the same methods.
    
*   In the example below, polymorphism is shown through the shared `specialMove()` method of the `Dragon` and `Unicorn` classes. Both utilize the method in different ways, but can be handled in the same way.
...
*   Using inheritance, I would designate `Pets` as a superclass, and have `Cats`, `Dogs`, and `Birds` as subclasses extending from `Pets` because they are all types of pets. I would make `name`, `energy level`, `happiness level`, and `sleep` all properties of the `Pets` class, and let `Cats`, `Dogs`, and `Birds` inherit those properties from it. Then, I would create unique methods for each of the three pet types with `hunt` for `Cats`, `chase` for `Dogs`, and `fly` for `Birds`.
    
*   One advantage of using inheritance is that you don't have to redefine properties already defined in the superclass, and the subclasses can utilize them with the `super()` keyword.